...
var { generateAvatarURL2 } = require('../letter-avatars')
...
module.exports = function (sequelize, DataTypes) {
  ...
  User.getProfile = function (user) {
    ...
    return user.profile ? User.parseProfile(user.profile) : (user.email ? User.parseProfileByEmail(user.email, user.image) : null)
  }
  ...
  User.parseProfileByEmail = function (email, image) {
    return {
      name: email.substring(0, email.lastIndexOf('@')),
      photo: generateAvatarURL2('', email, image, false),
      biggerphoto: generateAvatarURL2('', email, image, true)
    }
  }
